...
export default withApiAuthRequired(async function model(req, res) {
	...
	const seriesEl = series ? series : undefined
	const where = userText + yearText + seriesText
	const vals = [userEl, yearEl, seriesEl].filter(x => x)
	switch (method) {
		case 'GET':
		...
		case 'POST':
			...
			const paramObject = {
				user: festUser.id,
				series: body && body.series ? body.series : '',
