...
export default withApiAuthRequired(async function model(req, res) {
	...
	const lineupUrlText = lineupUrl ? ' AND lineupUrl=?' : ''
	const lineupUrlEl = lineupUrl ? lineupUrl : undefined
	const posterUrlText = posterUrl ? ' AND posterUrl=?' : ''
	const posterUrlEl = posterUrl ? posterUrl : undefined
	const where = fgidText + lineupUrlText + posterUrlText
	const vals = [fgidEl, lineupUrlEl, posterUrlEl].filter(x => x)
	switch (method) {
		case 'GET':
